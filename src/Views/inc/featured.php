 ...
 <div class="container pb-16 mt-8">
     ...
     <div class="grid lg:grid-cols-4 sm:grid-cols-2 gap-6 auto-rows-fr">
         <?php
            if ($data['featuredProd']) {
                foreach ($data['featuredProd'] as $fp) {
            ?>
                 <!-- single product -->
                 <div class="group rounded bg-white shadow overflow-hidden">
                     ...
                     <div class="relative h-[240px] overflow-hidden">
                         <?php
                            if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/' . $fp['image'])) : ?>
                             <img src="<?= $fp['image'] ?>" class="w-full h-full object-cover">
                         <?php else : ?>
                             <img src="src/uploads/<?= $fp['image'] ?>" class="w-full h-full object-cover">
                         <?php endif; ?>
                         <div class="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition">
                             <a href="details?id=<?= $fp['id'] ?>" class="text-white text-lg w-9 h-9 rounded-full bg-primary hover:bg-gray-800 transition flex items-center justify-center">
                     ...
                     <div class="pt-4 pb-3 px-4">
                         ...
                         <!-- product button -->
                         <a href="details?id=<?= $fp['id'] ?>" class="hidden group-hover:block group-hover:animate-fadeIn w-full py-1 text-center text-white bg-primary border border-primary rounded-b hover:bg-transparent hover:text-primary transition">
                             View Details
                         </a>
                         <!-- product button end -->
                     </div>
